...
class AddCommentTestCase(StaticLiveServerTestCase):
    ...
    def test_add_comment_to_post(self):
        ...
        driver.get(self.live_server_url + f'/post/{post_id}')

        comment_text = driver.find_element(By.TAG_NAME, 'textarea')

        # Wait for comment to be added
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CLASS_NAME, 'individual-comment'))
        )

        # Find comment content
        comment_content = driver.find_element(By.XPATH, "//div[@class='individual-comment']/span[1]")
        self.assertEqual(comment_content.text, 'Sample Comment')